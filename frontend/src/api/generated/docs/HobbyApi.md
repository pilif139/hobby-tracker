...
const { status, data } = await apiInstance.deleteHobbyRemoveFromProfileByHobbyId(
    hobbyId
);
```
...
const { status, data } = await apiInstance.getHobby(
    search,
    offset,
    limit
);
...
const { status, data } = await apiInstance.postHobby(
    postHobbyRequest
);
```
...
const { status, data } = await apiInstance.postHobbyAddToProfileByHobbyId(
    hobbyId
);
```
